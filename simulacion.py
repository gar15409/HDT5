...
random_seed = 1484 	# Numero arbitrario permite generar siempre los mismos aleatorios
cantidad_PRCS = 25	# Ejecucion de X procesos
capacidad_PRCS = 3.0	# Intervalo
...
cantidad_MEM = 10      # Cada proceso utiliza maximo de memoria 10
		...
		while (ins > 0): # Se ejecutara mientras existan instrucciones en cola
			with unit.request() as req2:
				...
				if (ins < 3):
					...
					term_PRCS = env.now
					temp_PRCS_time = term_PRCS 	# Variable temporal para guardar la operacion
					...
					ins = ins-3	# Se restan las instrucciones disponibles a instrucciones
...
def procesamiento(env, cantidad, capacidad, unit, io, ram):
	...
	for i in range(cantidad):
		mem2 = random.randint(1, cantidad_MEM)						# Random para memoria del proceso
		istr = random.randint(1, cantidad_INS)						# Random para instrucciones del proceso
		nuevo_proceso = newProceso(env, ('%s' % i), unit, ram, io, mem2, istr)		# Se crea un nuevo proceso
...
unit = simpy.Resource(env, capacity = 1)
ram_TOT = simpy.Container(env, capacity = 100, init = 100)
io = simpy.Resource(env, capacity = 1)
env.process(procesamiento(env, cantidad_PRCS, capacidad_PRCS, unit, io, ram_TOT))
